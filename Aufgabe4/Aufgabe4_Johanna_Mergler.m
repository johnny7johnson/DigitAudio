...
counter = 1;
%convoluted = zeros(length(castanetes)+length(HRIR)-1, length(degrees)*2);            %init array for time conv
convoluted = zeros(length(castanetes), length(degrees)*2);            %init array for frequency conv
...
for d = degrees
    ...
    cast1_l=convViaFFT(currentChannel(:,1), castanetes(:,1));
    cast2_r=convViaFFT(currentChannel(:,2), castanetes(:,2));             
